...
class Classifier:
	...
	var = None   #各分量的方差
	title_simi = None
	categories_simi = None
	baseInfoKeyList_simi = None
	baseInfoValueList_simi = None

	categories_IDF = None   # 逆文本频率指数  用于给开发类别和属性名加权
	baseInfoKeyList_IDF = None

	categories_num = None   # 拥有开放类别和基本属性的 item数量
	baseInfoKeyList_num = None

	# 相似度权值，分别为：title，categories，baseInfoKeyList，baseInfoValueList，detail
	...
	def load_trainSet(self,HudongList):  # 传入已经标注过的csNode列表
		...
		for p in self.labeled_hudongList:
			...
			for t in p.categories:
				if t in self.categories_IDF:
					self.categories_IDF[t] += 1
				else:
					self.categories_IDF[t] = 1
		...
		for p in self.categories_IDF:
			self.categories_IDF[p] = log(1.0 * self.categories_num / self.categories_IDF[p])
			#print(str(p)+"---"+str(self.categories_IDF[p]))
	...
	def get_categories_simi(self,item1,item2):
		categories_simi = 0.0
		L1 = item1.categories[:10]  #取前10个开放类别足够
		L2 = item2.categories[:10]
...
#			baseInfoValueList_simi /= len(and12)
	...
	def similarity(self,item1,item2):  # 比较两个页面的相似度，返回[-1,1]之间的相似度
		...
		categories_simi = 0.0
		for p1 in item1.categories:   #两组开放类别之间两两比较相似度，求和，求平均
			for p2 in item2.categories:
				categories_simi += self.model.similarity(p1,p2)
		fm = len(item1.categories)*len(item2.categories)
		if fm > 0:
			categories_simi /= fm
...
#		detail_simi = self.model.similarity(d1,d2)
		...
		# 各组相似度线性加权
		simi = self.weight[0]*title_simi + self.weight[1]*categories_simi + self.weight[2]*baseInfoKeyList_simi + self.weight[3]*baseInfoValueList_simi
	...
	def KNN_predict(self,item): # 预测互动页面的类别
		...
		for p in self.labeled_hudongList:  # 预先计算存储各分量相似度
			...
			categories_simi.append(self.get_categories_simi(p, item))
		...
		for p in self.labeled_hudongList: # 计算方差
			...
			var[1] += (categories_simi[i]-mean[1])*(categories_simi[i]-mean[1])
		...
		# 对于没有categories的 ，赋予平均值
		# 对title和categories进行高斯归一，对后面两项进行maxmin归一
		i = 0
		for p in self.labeled_hudongList:
			title_simi[i] = (title_simi[i]-mean[0])/stand[0]

			if categories_simi[i] == 0.0: #对于没有出现的，赋予平均值
				categories_simi[i] = mean[1]
			categories_simi[i] = (categories_simi[i]-mean[1])/stand[1]
		...
		for p in self.labeled_hudongList: # 计算各项相似度的加权和
			s = self.weight[0]*title_simi[i] + self.weight[1]*categories_simi[i] + self.weight[2]*baseInfoKeyList_simi[i] + self.weight[3]*baseInfoValueList_simi[i]
			count += 1
			if count < 2:
				pass
			#	print(str(title_simi[i])+" "+str(categories_simi[i])+" "+str(baseInfoKeyList_simi[i])+" "+str(baseInfoValueList_simi[i]))
			i += 1
			l = p.label
			t = p.title
